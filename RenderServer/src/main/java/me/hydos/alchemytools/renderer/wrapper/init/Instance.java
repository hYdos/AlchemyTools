...
public class Instance implements Closeable, VkWrapper<VkInstance> {
    ...
    public Instance(boolean enableDebug, boolean windowAvailable, VulkanCreationContext provider) {
        LOGGER.info("Creating VkInstance");
        try (var stack = MemoryStack.stackPush()) {
            var appInfo = VkApplicationInfo.calloc(stack)
                    .sType$Default()
                    .pApplicationName(stack.UTF8(provider.appName))
                    .pEngineName(stack.UTF8(provider.engineName))
            ...
            VkUtils.ok(VK10.vkCreateInstance(creationInfo, null, pInstance), "Failed creating instance");
            this.instance = new VkInstance(pInstance.get(0), creationInfo);

            if (enableDebug) {
                var pHandle = stack.mallocLong(1);
                VkUtils.ok(EXTDebugUtils.vkCreateDebugUtilsMessengerEXT(instance, debugCallback, null, pHandle), "Failed creating debug utils");
